    ...
    function ajax_custom_login() {
    	...
    	// first check if data is being sent and that it is the data we want
      	if ( isset( $_POST["username"] ) && isset( $_POST["password"] ) ) {
            ...
            if (is_wp_error( $wpUser ) ) \BCController::ajaxError('Error retrieving the wordpress user: '.$wpUser->get_error_message());
            else if (!$wpUser) \BCController::ajaxError('Error retrieving the wordpress user');
            		
            wp_clear_auth_cookie();
    		wp_set_current_user( $wpUser->ID, $wpUser->user_login );
            wp_set_auth_cookie  ( $wpUser->ID );
        
			\BCController::ajaxSuccess(get_permalink(get_page_by_path( 'my-courses' )));
    	}
    	
        \BCController::ajaxError('There was an error in the autetication process');
    }
