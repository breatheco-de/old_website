...
var _messaging = __webpack_require__(33);
...
var StudentProfile = function () {
    ...
    _createClass(StudentProfile, [{
        ...
        value: function init() {
            ...
            document.querySelector('#modal-enable_quiz .send-btn').addEventListener("click", function (btn) {
                var quizSlug = $('#quiz-select').val();
            ...
            document.querySelector('#modal-give_badge_points .send-btn').addEventListener("click", function (btn) {
                var badgeId = $('#badges').val();
                var studentId = $('#student-id').val();
                var points = $('#points-given').val();

                if (!_validator2.default.isNumeric(points) || parseInt(points) == 0) _messaging.BCMessaging.addMessage(_messaging.BCMessaging.ERROR, 'You need to specify the number of points you want to give');
                if (_validator2.default.isEmpty(badgeId) || parseInt(badgeId) == 0) _messaging.BCMessaging.addMessage(_messaging.BCMessaging.ERROR, 'Please select a badge');

                var messages = _messaging.BCMessaging.getMessages(_messaging.BCMessaging.ERROR);
                if (messages.length > 0) _messaging.BCMessaging.notifyPending(_messaging.BCMessaging.ERROR);else {
                    _this.sendForm({
                        action: 'give_points',
                        badge: badgeId,
                        student: studentId,
            ...
            document.querySelector('#give-points').addEventListener("click", function (btn) {

                if (_this.badges && _this.badges.length > 0) _this.fillModalWithBadges();else _this.getBadges();
            });
        ...
        value: function sendForm(thedata) {
            ...
            $.ajax({
                ...
                success: function success(response) {
                    if (response) {
                        if (response.code == '200') {
                            if (_success) _success();else window.location.reload();
                        } else {
                ...
                error: function error() {
                    _messaging.BCMessaging.notify(_messaging.BCMessaging.ERROR, "There was an error processing the request");
                }
            });
        }
    }, {
                    ...
                    if (response) {
                        if (response.code == '200') {
                            if (response.data && response.data.length > 0) {
                                _this2.badges = response.data;
                                _this2.fillModalWithBadges();

                                var badgesSelect = document.querySelector('#badges');
                                badgesSelect.addEventListener('change', function (evt) {

                                    var badge = _this2.getSingleBadge(evt.target.value);
                                    if (badge) {
                                        ...
                                        document.querySelector("#badge-title").innerHTML = _this2.htmlEntities(badge.name);
                                        document.querySelector("#badge-description").innerHTML = _this2.htmlEntities(badge.description);
                                    }
                                });
                            }
                        } else {
        ...
        key: 'fillModalWithBadges',
        value: function fillModalWithBadges() {
            var badgesSelect = document.querySelector('#badges');
        ...
        key: 'getSingleBadge',
        value: function getSingleBadge(slug) {
...
var _assertString = __webpack_require__(32);
...
var _isByteLength = __webpack_require__(40);
...
var _toDate = __webpack_require__(35);

var _toDate2 = _interopRequireDefault(_toDate);

var _toFloat = __webpack_require__(38);
...
var _isFullWidth = __webpack_require__(43);

var _isFullWidth2 = _interopRequireDefault(_isFullWidth);

var _isHalfWidth = __webpack_require__(44);
...
var _isHexadecimal = __webpack_require__(45);
...
var _ltrim = __webpack_require__(46);

var _ltrim2 = _interopRequireDefault(_ltrim);

var _rtrim = __webpack_require__(47);
...
var _blacklist = __webpack_require__(48);
...
var _toString = __webpack_require__(36);
